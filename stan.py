...
def calcul_impact_total(taille_mo, nb_tokens, techno, coeff_ia, duree_stockage_jours=30):
    ...
    # On utilise le coefficient IA qui dépend du GPU choisi
...
# --- SIDEBAR : CHOIX DES COMPOSANTS (APPEL D'OFFRE) ---
...
# On ajuste le coefficient de consommation selon la puissance du GPU
...
if uploaded_file is not None:
    ...
    if st.button("LANCER L'ANALYSE D'IMPACT"):
        ...
        with st.spinner("Mesure CodeCarbon en cours sur votre PC..."):
            tracker = EmissionsTracker(log_level="error")
            tracker.start()
            # Simulation d'une charge de travail
            sum(i*i for i in range(10**7)) 
            tracker.stop()
            conso_pc_wh = tracker.final_emissions_data.energy_consumed * 1000

        # Calculs avec les variables choisies
